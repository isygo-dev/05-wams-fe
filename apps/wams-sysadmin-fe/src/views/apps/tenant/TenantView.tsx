...
import { TenantDetail, EnumLinkTenant } from 'ims-shared/@core/types/ims/tenantTypes'

const schema = yup.object().shape({
  name: yup.string().required(),
  url: yup.string().matches(URL_PATTERN, 'Enter a valid url').required(),
  tenant: yup.string().default(''),
...
const TenantView = (props: TenantView) => {
  ...
  const [defaultValues, setDefaultValues] = useState<TenantDetail>({ ...tenantDetail })
  const [newStatus, setNewStatus] = useState<boolean>(
    tenantDetail && tenantDetail.adminStatus === 'ENABLED' ? true : false
  )
  const initAdress = {
    country: '',
    state: '',
  ...
  const [editedAddress, setEditedAddress] = useState<AddressTypes>(
    tenantDetail && tenantDetail?.address ? tenantDetail.address : initAdress
  )
  ...
  async function onSaveImage(newImage: Blob) {
    updatePictureMutation.mutate({ id: tenantDetail.id, file: newImage })
    setPhotoFile(newImage as File)
  }

  const updatePictureMutation = useMutation({
    mutationFn: (newMutation: { id: number; file: Blob }) => TenantApis(t).updateTenantImage(newMutation),
    onSuccess: () => {
  ...
  const handlereset = () => {
    setEditedAddress({ ...tenantDetail.address })
    reset()
  }

  const mutationEdit = useMutation({
    mutationFn: (data: TenantDetail) => TenantApis(t).updateTenant(data),
    onSuccess: (res: TenantDetail) => {
      console.log('res', res)
  ...
  const mutationEditSocial = useMutation({
    mutationFn: (data: { id: number; social: string; link: string }) =>
      TenantApis(t).updateTenantSocial(data.id, data.social, data.link),
    onSuccess: (res: TenantDetail) => {
      setDefaultValues(res)
      setErrorNewLinkValue(false)
      handleClickOpen()
  ...
  const mutationEditStatus = useMutation({
    ...
    onSuccess: res => {
      const cachedData: TenantDetail | undefined = queryClient.getQueryData('tenantDetail')
      if (cachedData != undefined) {
        console.log(cachedData)
        cachedData.adminStatus = res.newReqStatus == 'DISABLED' ? AdminStatus.DISABLED : AdminStatus.ENABLED
        queryClient.setQueryData('tenantDetail', cachedData)
      }
    }
  })
  ...
  const {
    ...
    setValue,
    formState: { errors }
  } = useForm<TenantDetail>({
    defaultValues, // Start with an empty object
    mode: 'onChange',
    resolver: yupResolver(schema)
  })

  const onSubmitDtata = (data: TenantDetail) => {
    const request: TenantDetail = {
      id: tenantDetail.id,
      ...data,
  ...
  const onSubmitCheck = () => {
    const data: RequestStatus = { id: tenantDetail.id ?? 0, newReqStatus: newStatus ? 'ENABLED' : 'DISABLED' }
  ...
  function onDelete() {
    switch (linkDeleted) {
      case EnumLinkTenant.lnk_facebook:
        defaultValues.lnk_facebook = ''
        break
      case EnumLinkTenant.lnk_linkedin:
        defaultValues.lnk_linkedin = ''
        break
      case EnumLinkTenant.lnk_xing:
        defaultValues.lnk_xing = ''
        break
    }
    onSubmitSocial(defaultValues.id, linkDeleted, '')
  }

  const onDeleteLink = (item: string) => {
    if (!checkPermission(PermissionApplication.IMS, PermissionPage.TENANT, PermissionAction.WRITE)) {
      return
    }
    setLinkDeleted(item)
  ...
  const handleSaveSocial = () => {
    if (linkAdded && linkAdded.trim() !== '' && URL_PATTERN.test(newLinkValue)) {
      ...
      onSubmitSocial(tenantDetail.id, linkAdded, newLinkValue)
      setErrorNewLinkValue(false)
    } else {
    ...
    <>
      <Grid container spacing={2}>
        <Grid item sm={12} md={2} xs={12}>
          <PictureCard
            photoFile={photoFile}
            url={`${imsApiUrls.apiUrl_IMS_Tenant_ImageDownload_EndPoint}/${tenantDetail.id}`}
      ...
      <Grid container spacing={6} mt={2}>
        <Grid item xs={12}>
          <Card>
            <CardHeader title={t('Tenant.Details')} />
            <form>
                                ...
                                >
                                  ...
                                  {checkPermission(
                                    ...
                                    <Button
                                      onClick={() => {
                                        defaultValues.lnk_facebook
                                          ? onDeleteLink(EnumLinkTenant.lnk_facebook)
                                          : onAddLink(EnumLinkTenant.lnk_facebook)
                                      }}
                                      variant='outlined'
                                ...
                                >
                                  ...
                                  {checkPermission(
                                    ...
                                    <Button
                                      onClick={() => {
                                        defaultValues.lnk_linkedin
                                          ? onDeleteLink(EnumLinkTenant.lnk_linkedin)
                                          : onAddLink(EnumLinkTenant.lnk_linkedin)
                                      }}
                                ...
                                >
                                  ...
                                  {checkPermission(
                                    ...
                                    <Button
                                      onClick={() => {
                                        defaultValues.lnk_xing
                                          ? onDeleteLink(EnumLinkTenant.lnk_xing)
                                          : onAddLink(EnumLinkTenant.lnk_xing)
                                      }}
        ...
        <Grid item md={12}>
          <AdminAccountsList tenant={defaultValues?.name} />
        </Grid>
        <Grid item xs={12}>
          {checkPermission(PermissionApplication.IMS, PermissionPage.TENANT, PermissionAction.WRITE) && (
            <Card>
                    ...
                    <FormControl>
                      <Controller
                        ...
                        render={({ field }) => (
                          <FormControlLabel
                            label={
                              tenantDetail && tenantDetail.adminStatus === 'ENABLED'
                                ? t('Tenant.I_confirm_my_tenant_deactivation')
                                : t('Tenant.I_confirm_my_tenant_activation')
                            }
                      ...
                      {errorsCheck.checkbox && (
                        <FormHelperText sx={{ color: 'error.main' }} id='validation-basic-checkbox'>
                          {tenantDetail && tenantDetail.adminStatus == 'DISABLED'
                            ? t('Tenant.Please_confirm_you_want_to_activate_tenant')
                            : t('Tenant.Please_confirm_you_want_to_deactivate_tenant')}
                        </FormHelperText>
                      )}
                    </FormControl>
                  </Box>
          ...
          <DialogContentText id='alert-dialog-slide-description'>
            <Grid container>
              <Grid item md={1} sm={1} xs={1}>
                {linkAdded && linkAdded === EnumLinkTenant.lnk_facebook ? (
                  <img src={'/images/logos/facebook.png'} alt={'facebook'} height='30' />
                ) : null}
                {linkAdded && linkAdded === EnumLinkTenant.lnk_linkedin ? (
                  <img src={'/images/logos/linkedin.png'} alt={'linkedin'} height='30' />
                ) : null}
                {linkAdded && linkAdded === EnumLinkTenant.lnk_xing ? (

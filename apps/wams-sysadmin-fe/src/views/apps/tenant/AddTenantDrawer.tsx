...
import { Tenant, TenantRequest } from 'ims-shared/@core/types/ims/tenantTypes'
...
const SidebarAddTenant = (props: SidebarAddTenantType) => {
  ...
  const mutationAdd = useMutation({
    mutationFn: (data: FormData) => TenantApis(t).addTenant(data),
    onSuccess: (res: Tenant) => {
      handleClose()
      const cachedData: Tenant[] = queryClient.getQueryData('tenants') || []
      const updatedData = [...cachedData]
      updatedData.push(res)
      queryClient.setQueryData('tenants', updatedData)
    },
    onError: err => {
      console.log(err)
    }
  ...
  let defaultValues: TenantRequest = {
    name: '',
    url: '',
    email: '',
    phone: '',
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h6'>{t('Tenant.Add_Tenant')}</Typography>
        <IconButton
          size='small'
          onClick={handleClose}

...
const AccountStatisticsContainer: React.FC = () => {
  ...
  } = useQuery(['totalCount'], () => AccountStatApis(t).getGlobalStatistics(AccountStatEnum.TOTAL_COUNT));
  const {
    data: activeCount,
    isLoading: isLoadingActiveCount
  } = useQuery(['activeCount'], () => AccountStatApis(t).getGlobalStatistics(AccountStatEnum.ACTIVE_COUNT));
  const {
    data: confirmedCount,
    isLoading: isLoadingConfirmedCount
  } = useQuery(['confirmedCount'], () => AccountStatApis(t).getGlobalStatistics(AccountStatEnum.CONFIRMED_COUNT));
  const {
    data: adminsCount,
    isLoading: isLoadingAdminsCount
  } = useQuery(['adminsCount'], () => AccountStatApis(t).getGlobalStatistics(AccountStatEnum.ADMINS_COUNT))

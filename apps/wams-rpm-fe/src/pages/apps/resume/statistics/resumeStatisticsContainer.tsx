...
const ResumeStatisticsContainer: React.FC = () => {
  ...
  const { data: totalCount, isLoading: isLoadingTotalCount } = useQuery(['totalCount'], () =>
    ResumeStatApis(t).getGlobalStatistics(ResumeStatEnum.TOTAL_COUNT)
  )
  const { data: uploadedByMeCount, isLoading: isLoadingUploadedByMeCount } = useQuery(['uploadedByMeCount'], () =>
    ResumeStatApis(t).getGlobalStatistics(ResumeStatEnum.UPLOADED_BY_ME_COUNT)
  )
  const { data: confirmedCount, isLoading: isLoadingConfirmedCount } = useQuery(['confirmedCount'], () =>
    ResumeStatApis(t).getGlobalStatistics(ResumeStatEnum.CONFIRMED_COUNT)
  )
  const { data: completedCount, isLoading: isLoadingCompletedCount } = useQuery(['completedCount'], () =>
    ResumeStatApis(t).getGlobalStatistics(ResumeStatEnum.COMPLETED_COUNT)
  )
  const { data: interviewedCount, isLoading: isLoadingInterviewedCount } = useQuery(['interviewedCount'], () =>
    ResumeStatApis(t).getGlobalStatistics(ResumeStatEnum.INTERVIEWED_COUNT)
  )
  ...
  return (
    <Grid container spacing={2} sx={{ mb: 2 }}>
      ...
      <Grid item xs={12} sm={2.4}>
        <StatisticCard
          title='Statistic.Uploaded_By_Me'

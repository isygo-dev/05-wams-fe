...
function GridView(props: Props) {
  const {
    filteredStates, onDragEnd,
    filteredItems, openAddEventModal,
  } = props;


  return (
    <Card className='bloc-list-workflow' style={{margin: '20px 0', width: '100%'}}>
      <Box style={{height: ' max-content'}} sx={{
        border: 0, boxShadow: 0, color: 'inherit', display: 'flex', margin: '16px'
      }}>
...

          {
            filteredStates?.map((column, i) => (
              <Droppable key={i} droppableId={column.code} sx={{gap: 2}}>
                {(provided) => (
                  <div className='bloc-workflow'
                       {...provided.droppableProps}
                       ref={provided.innerRef}
                  >
                    <Card className="center-sticky">
                      <h2 className='bloc-title'
                      ...
                      .map((card, index) => (
                        ...
                        >
                          {(provided: any) => (
                            ...
                            >
                              <DummyCard data={card} color={column.color}
                                         openAddEvent={openAddEventModal}/>
                            </div>
                          )}
                        </Draggable>

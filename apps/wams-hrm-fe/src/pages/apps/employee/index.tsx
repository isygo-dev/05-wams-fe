...
const EmployeeList = () => {
  ...
  const { data: countEmployee, isLoading: isLoadingCountEmployee } = useQuery(`countEmployee`, () =>
    EmployeeApis(t).getEmployeesCount()
  )

  const { data: employee, isLoading } = useQuery(`employee`, () =>
    EmployeeApis(t).getEmployeesByPage(paginationModel.page, paginationModel.pageSize)
  )
        ...
        return (
          <>
            {checkPermission(PermissionApplication.IMS, PermissionPage.ACCOUNT, PermissionAction.WRITE) ? (
              <Switch
                size={'small'}
                checked={row.employeeStatus === EmployeeStatus.ENABLED}
      ...
      <Grid container>
        <Grid item xs={12} md={12} sm={12}>
          <Card>
            ...
            />
            {checkPermission(PermissionApplication.HRM, PermissionPage.EMPLOYEE, PermissionAction.READ) &&

...
const EmployeeStatisticsContainer: React.FC = () => {
  ...
  const { data: totalCount, isLoading: isLoadingTotalCount } = useQuery(['totalCount'], () =>
    EmployeeStatApis(t).getGlobalStatistics(EmployeeStatEnum.TOTAL_COUNT)
  )
  const { data: activeCount, isLoading: isLoadingActiveCount } = useQuery(['activeCount'], () =>
    EmployeeStatApis(t).getGlobalStatistics(EmployeeStatEnum.ACTIVE_COUNT)
  )
  const { data: confirmedCount, isLoading: isLoadingConfirmedCount } = useQuery(['confirmedCount'], () =>
    EmployeeStatApis(t).getGlobalStatistics(EmployeeStatEnum.CONFIRMED_COUNT)
  )

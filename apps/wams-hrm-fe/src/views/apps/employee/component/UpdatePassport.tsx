...
export const UpdatePassport: React.FC<UpdatePassportType> = ({open, toggle, dataParameter, refetch}) => {
  ...
  const defaultValues: Passport = {
    id: dataParameter?.details?.passport[0]?.id || null,
    cardNumber: dataParameter?.details?.passport[0]?.cardNumber || '',
    code: dataParameter?.details?.passport[0]?.code || '',
    issuedDate: dataParameter?.details?.passport[0]?.issuedDate || '',
  ...
  const updatePictureMutation = useMutation({
    mutationFn: (data: FormData) => EmployeeApis(t).updateEmployeeTravelDocImage(data, dataParameter?.details?.passport[0]?.id),
    onSuccess: () => {
      handleClose()
      refetch()
    },
  ...
  const onSubmitCin = async (data: Passport) => {
    const formData = new FormData()
    if (data.cardNumber != null && data.cardNumber !== undefined) {
      formData.set('cardNumber', data.cardNumber)
    }
    if (
      dataParameter?.details?.passport[0]?.id.toString() != null &&
      dataParameter?.details?.passport[0]?.id.toString() !== undefined
    ) {
      formData.set('id', dataParameter?.details?.passport[0]?.id.toString())
    }
    if (dataParameter?.details?.passport[0]?.code != null && dataParameter?.details?.passport[0]?.code !== undefined) {
      formData.set('code', dataParameter?.details?.passport[0]?.code)
    }
    ...
    if (selectedFile) {
      ...
      formData.append('imagePath', dataParameter?.details?.passport[0]?.imagePath)
    }
          ...
          <Grid container spacing={3}>
                ...
                <Controller
                  ...
                  render={({field: {value, onChange}}) => (
                    <TextField
                      ...
                      label={t('CardNumber')}
                      onChange={onChange}
                      defaultValue={dataParameter?.details?.passport[0]?.cardNumber}
                ...
                <Controller
                  ...
                  render={({field: {value, onChange}}) => (
                    <TextField
                      ...
                      label={t('issuedPlace')}
                      onChange={onChange}
                      defaultValue={dataParameter?.details?.passport[0]?.issuedPlace}
            ...
            <Grid item xs={12} sm={6}>
              <DatePickerWrapper className='small-input-data'>
                <Controller
                  name='issuedDate'
                  control={control}
                  defaultValue={dataParameter?.details?.passport[0]?.issuedDate || null}
                  render={({field: {value, onChange}}) => (
                    <DatePicker
            ...
            <Grid item xs={12} sm={6}>
              <DatePickerWrapper className='small-input-data'>
                <Controller
                  name='expiredDate'
                  control={control}
                  defaultValue={dataParameter?.details?.passport[0]?.expiredDate || null}
                  render={({field: {value, onChange}}) => (
                    <DatePicker
                      selected={value ? new Date(value) : null}
                      dateFormat='dd/MM/yyyy'
            ...
            <Grid item xs={12}>
              <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '20px'}}>
                {(selectedFile != null ||
                  undefined ||
                  dataParameter?.details?.passport[0]?.imagePath != null ||
                  undefined) && (
                  <img
                    src={
                      selectedFile
                        ? URL.createObjectURL(selectedFile)
                        : dataParameter?.details?.passport[0]?.imagePath
                          ? `${hrmApiUrls.apiUrl_HRM_TravelDoc_ImageDownload_EndPoint}/${dataParameter?.details?.passport[0]?.id}`
                          : undefined
                    }
                    alt={selectedFile ? '' : dataParameter?.details?.passport[0]?.imagePath ? 'Passport Image' : ''}
                    style={{
                      cursor: 'pointer',
                      width: '400px',
                      height: '150px',

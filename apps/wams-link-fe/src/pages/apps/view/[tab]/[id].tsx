...
const Detail = () => {
  ...
  const {data: contactsAccount} = useQuery(['contactsAccount', user?.domain], () => AccountApis(t).getAccountsByDomain(user?.domain))
  ...
  } = useQuery(`dataDetail`, () => id && PostApis(t).getPostById(Number(id)))

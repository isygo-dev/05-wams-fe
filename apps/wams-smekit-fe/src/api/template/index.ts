...
export const getTemplatesByPage = async (page: number, size: number) => {
  try {
    const response = await AppQuery(`${apiUrls.apiUrl_smekit_Template_FetchAll_Endpoint}/${page}/${size}`, {
      method: 'GET',
      headers: {
...
export const getTemplatePreview = async (templateId, version = 1) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.message || `Failed to get template preview: ${response.status} - ${response.statusText}`
      )
    }
    ...
    throw new Error(
      error.message || "Une erreur inattendue est survenue lors de la récupération de l'aperçu du template"
    )
  }
}
...
export const updateTemplate = async (formData: FormData) => {
  ...
  const response = await AppQuery(`${apiUrls.apiUrl_smekit_Template_StorageConfigEndpoint}?id=${id}`, {
    method: 'PUT',
    body: formData,
    headers: {
      Accept: 'application/json'

...
export const getAuthorTemplates = async (authorId: number): Promise<TemplateType[]> => {
  ...
  const response = await AppQuery(`${apiUrls.apiUrl_smekit_Template_FetchAll_Endpoint}?authorId=${authorId}`, {
    method: 'GET',
    headers: {
      Accept: 'application/json'
    }
...
export const uploadAuthorFile = async (file: File, authorId: number) => {
  ...
  const response = await AppQuery(`${apiUrls.apiUrl_Author_File_Upload}?id=${authorId}`, {
    method: 'PUT',
    headers: {
      Accept: 'application/json',
      'Access-Control-Allow-Origin': '*'

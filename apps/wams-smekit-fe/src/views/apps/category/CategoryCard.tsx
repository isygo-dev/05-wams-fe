...
const CategoryCard = (props: CardItem) => {
  ...
  const hasWritePermission = checkPermission(PermissionApplication.IMS, PermissionPage.ACCOUNT, PermissionAction.WRITE)
            ...
            {data?.tagName && data.tagName.length > 0 ? (
              <Stack direction='row' spacing={1} flexWrap='wrap' gap={1}>
                {data.tagName.map((tag, index) => (
                  <CustomChip key={index} label={tag.tagName} color='primary' size='small' skin='light' rounded />
                ))}
              </Stack>
            ) : (
      ...
      <Box sx={{ flex: '0 0 auto' }}>
        ...
        >
          <Stack direction='row' spacing={1}>
            <CustomChip rounded size='small' skin='light' color={typeConfig.color} label={typeConfig.label} />
          </Stack>

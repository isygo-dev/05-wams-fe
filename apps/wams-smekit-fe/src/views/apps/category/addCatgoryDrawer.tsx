...
import {
  Switch, FormControlLabel, Avatar, InputLabel, Select,
  MenuItem, FormHelperText, Chip, Autocomplete
} from "@mui/material";
...
const AddCategoryDrawer = ({ category, showDialogue, setShowDialogue }) => {
  ...
  const getImageSource = () => {
    ...
    return currentImage ||
      (category?.id ? `${apiUrls.apiUrl_smekit_Category_ImageDownload_Endpoint}/${category.id}?${Date.now()}` : undefined);
  };
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Autocomplete
              ...
              filterOptions={(options, params) => {
                ...
                const filtered = options.filter(option =>
                  (option.tagName?.toLowerCase() || '').includes(input)
                )
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Button
              ...
              variant='contained'
              sx={{ mr: 4 }}
              disabled={updatePictureMutation.isLoading || addCategoryMutation.isLoading || updateCategoryMutation.isLoading}
            >
              {t('Submit')}
            </Button>
            <Button

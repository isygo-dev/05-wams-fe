...
const CategorySelector = ({ selectedCategory, setSelectedCategory, categories = [] }: CategorySelectorProps) => {
  ...
  const {
    ...
    isLoading,
    error
  } = useQuery('allTemplates', fetchAllTemplate, {
    select: data => {
  ...
  const countTemplatesByCategory = React.useCallback(
      ...
      >
        <Select
          ...
          renderValue={selected => (
            ...
            >
              <Icon icon='mdi:tag-outline' fontSize='1rem' color={selected ? 'primary' : 'disabled'} />
              <Typography variant='body2' noWrap sx={{ flex: 1 }}>
                {selected ? categories.find(c => c.id === selected)?.name : t('All categories')}
              </Typography>
              <Chip

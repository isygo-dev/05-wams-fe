...
// ** Types
import {ConnectionsTabType} from 'template-shared/@fake-db/types'
                            ...
                            <Card sx={{position: 'relative'}}>
                                ...
                                <CardContent sx={{pt: 9.5}}>
                                    <Box sx={{display: 'flex', alignItems: 'center', flexDirection: 'column'}}>
                                        ...
                                        <Typography
                                            sx={{mb: 5, color: 'text.secondary'}}>{item.designation}</Typography>
                                        <Box sx={{mb: 5, display: 'flex', alignItems: 'center'}}>
                                            {item.chips &&
                                                item.chips.map((chip, index) => (
                                                    ...
                                                    >
                                                        <CustomChip rounded size='small' skin='light' color={chip.color}
                                                                    label={chip.title}/>
                                                    </Box>
                                                ))}
                                        </Box>
                                        ...
                                        <Box sx={{display: 'flex', alignItems: 'center'}}>
                                            <Button sx={{mr: 4, '& svg': {mr: 2}}}
                                                    variant={item.isConnected ? 'contained' : 'outlined'}>
                                                <Icon fontSize='1.125rem'
                                                      icon={item.isConnected ? 'tabler:user-check' : 'tabler:user-plus'}/>

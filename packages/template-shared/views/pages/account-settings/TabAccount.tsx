...
const TabAccount = (props: PropsAccountDetailType) => {
    ...
    const {data: rolesList} = useQuery('roles', RolePermissionApis(t).getRoles)
    ...
    useEffect(() => {
        if (user) {
            ...
            setPhoto(`${imsApiUrls.apiUrl_IMS_Account_ImageDownload_EndPoint}/${user.id}?` + new Date().getTime())
    ...
    useQuery(
        'avatar',
        () => (user?.id ? `${imsApiUrls.apiUrl_IMS_Account_ImageDownload_EndPoint}/${user.id}?` + new Date().getTime() : null),
        {
    ...
    const handleToggle = (value: RoleTypes) => () => {
        if (!checkPermission(PermissionApplication.IMS, PermissionPage.ACCOUNT_DETAIL, PermissionAction.WRITE)) {
    ...
    const saveProfileData = () => {
        const editedProfile: AccountDto = {...user}
    ...
    async function onSaveImage(newImage: Blob) {
        if (user && user.id) {
            const result = AccountApis(t).uploadPicture({id: user.id, file: newImage})
            if (result) {
                queryClient.setQueryData(
                    'avatar',
                    `${imsApiUrls.apiUrl_IMS_Account_ImageDownload_EndPoint}/${user.id}?` + new Date().getTime()
                )
            } else {
                queryClient.setQueryData(
                    'avatar',
    ...
    async function updateProfile(account: AccountDto) {
        if (!id) {
            const editAccount: AccountDto = await AccountApis(t).updateProfile(account)
            ...
            AccountApis(t).updateUserAccount({id, account})
        }
    }
                                    ...
                                    <Grid container spacing={5}>
                                        ...
                                        <Grid item xs={12} sm={6}>
                                            <FormControl fullWidth>
                                                ...
                                                >
                                                    <MenuItem value='gmt-12'>(GMT-12:00) International Date Line
                                                        West</MenuItem>
                                                    <MenuItem value='gmt-11'>(GMT-11:00) Midway Island, Samoa</MenuItem>
                                                    <MenuItem value='gmt-10'>(GMT-10:00) Hawaii</MenuItem>
                                                    <MenuItem value='gmt-09'>(GMT-09:00) Alaska</MenuItem>
                                                    <MenuItem value='gmt-08'>(GMT-08:00) Pacific Time (US &
                                                        Canada)</MenuItem>
                                                    <MenuItem value='gmt-08-baja'>(GMT-08:00) Tijuana, Baja
                                                        California</MenuItem>
                                                    <MenuItem value='gmt-07'>(GMT-07:00) Chihuahua, La Paz,
                                                        Mazatlan</MenuItem>
                                                    <MenuItem value='gmt-07-mt'>(GMT-07:00) Mountain Time (US &
                                                        Canada)</MenuItem>
                                                    <MenuItem value='gmt-06'>(GMT-06:00) Central America</MenuItem>
                                                    <MenuItem value='gmt-06-ct'>(GMT-06:00) Central Time (US &
                                                        Canada)</MenuItem>
                                                    <MenuItem value='gmt-06-mc'>(GMT-06:00) Guadalajara, Mexico City,
                                                        Monterrey</MenuItem>
                                                    <MenuItem value='gmt-06-sk'>(GMT-06:00) Saskatchewan</MenuItem>
                                                    <MenuItem value='gmt-05'>(GMT-05:00) Bogota, Lima, Quito, Rio
                                                        Branco</MenuItem>
                                                    <MenuItem value='gmt-05-et'>(GMT-05:00) Eastern Time (US &
                                                        Canada)</MenuItem>
                                                    <MenuItem value='gmt-05-ind'>(GMT-05:00) Indiana (East)</MenuItem>
                                                    <MenuItem value='gmt-04'>(GMT-04:00) Atlantic Time
                                                        (Canada)</MenuItem>
                                        ...
                                        <Grid item xs={12} sm={12}>
                                            ...
                                            <FormControl fullWidth>
                                                    ...
                                                    <List>
                                                        {user?.roleInfo &&
                                                            user?.roleInfo?.map((tool: RoleTypes, index: number) => (
                                                                <Tooltip title={t(tool?.description ?? '') as string}
                                                                         key={index}>
                                                ...
                                                {errorMessage && <FormHelperText
                                                    sx={{color: 'error.main'}}>{errorMessage}</FormHelperText>}
                                            </FormControl>
                                        </Grid>

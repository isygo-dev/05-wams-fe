                                ...
                                {editedData.resumeShareInfos?.map((sharedData: ResumeShareInfo) => {
                                    return (
                                        <Grid key={sharedData.id} container direction='row' spacing={2} mt={1}>
                                            <Grid item>
                                                <Avatar
                                                    src={`${imsApiUrls.apiUrl_IMS_Account_ImageDownload_EndPoint}/${getMiniAccountByCode(sharedData.sharedWith).id}`}
                                            ...
                                            {checkPermission(PermissionApplication.RPM, PermissionPage.RESUME_SHARE_INFO, PermissionAction.WRITE) &&

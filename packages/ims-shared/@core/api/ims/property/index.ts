...
const PropertyApis = (t: TFunction) => {
  ...
  const getPropertyByAccountAndGuiAndName = async (accountCode: string, guiName: string, name: string) => {
    ...
    const response = await AppQuery(
      `${imsApiUrls.apiUrl_IMS_Property_Account_Gui_Name_EndPoint}?accountCode=${accountCode}&guiName=${guiName}&name=${name}`,
      {
        method: 'GET',
        headers: {
  ...
  const getPropertiesByAccountAndGui = async (accountCode: string, guiName: string) => {
    ...
    const response = await AppQuery(
      `${imsApiUrls.apiUrl_IMS_Property_Account_Gui_All_EndPoint}?accountCode=${accountCode}&guiName=${guiName}`,
      {
        method: 'GET',
        headers: {
          Accept: 'application/json',

...
const ResumeAdditionalFilesApis = (t: TFunction) => {
    ...
    const uploadAdditionalFiles = async (data: { parentId: number; files: Blob[] }) => {
        ...
        const response = await AppQuery(`${rpmApiUrls.apiUrl_RPM_Resume_MultiFilesUpload_EndPoint}?parentId=${data.parentId}`, {
            method: 'PUT',
            headers: {
    ...
    const downloadAdditionalFile = async (data: {
        ...
        const response = await AppQuery(`${rpmApiUrls.apiUrl_RPM_Resume_MultiFilesDownload_EndPoint}?parentId=${data.parentId}&fileId=${data.fileId}&version=${data.version}`, {
            method: 'GET',
            headers: {
                Accept: 'application/json',
                'Access-Control-Allow-Origin': '*'
    ...
    const deleteAdditionalFile = async (data: { parentId: number; fileId: number }) => {
        ...
        const response = await AppQuery(`${rpmApiUrls.apiUrl_RPM_Resume_MultiFilesDelete_EndPoint}?parentId=${data.parentId}&fileId=${data.fileId}`, {
            method: 'DELETE',
            headers: {
                Accept: 'application/json',
                'Access-Control-Allow-Origin': '*'

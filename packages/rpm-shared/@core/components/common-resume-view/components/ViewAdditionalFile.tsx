...
const ViewAdditionalFile = (props: AdditionalFilesDetails) => {
  ...
  const updateAdditionalFileMutation = useMutation({
    mutationFn: (data: { parentId: number; files: Blob[] }) => ResumeAdditionalFilesApis(t).uploadAdditionalFiles(data),
    onSuccess: (res: any) => {
      onDataFromChild(res)
    }
  })
      ...
      <div>
        ...
        {checkPermission(PermissionApplication.RPM, PermissionPage.RESUME_ADDITIONAL_INFO, PermissionAction.DELETE) && (
          <Tooltip title={t('Action.Delete')}>
            <IconButton size='small' sx={{ color: 'text.secondary' }} onClick={() => deleteAdditionalFiles(file.id)}>
            ...
            <CardContent
              sx={
                !checkPermission(
                  ...
                  PermissionPage.RESUME_ADDITIONAL_INFO,

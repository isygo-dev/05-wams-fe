...
const ApplicationjobOffer = (props: ApplicationjobOfferProps) => {
  ...
  const { data: jobOfferList } = useQuery([`jobOfferList`, code], () =>
    JobOfferApis(t).getJobOffersNotAssignedToResume(code)
  )
  ...
  return (
    ...
    >
      ...
      <Box sx={{ p: theme => theme.spacing(0, 6, 6) }}>
        <form>
          <TextField size='small' disabled={true} value={email || ''} label={t('Email')} fullWidth sx={{ mb: 4 }} />
          <FormControl fullWidth sx={{ mb: 4 }}>
            <InputLabel>{t('Job Offer')}</InputLabel>
            <Select size='small' name='jobOffre' onChange={e => handlejobOfferChange(e)} label={t('Job Offer')}>

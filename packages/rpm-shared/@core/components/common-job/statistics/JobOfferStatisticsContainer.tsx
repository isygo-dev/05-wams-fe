...
const JobOfferStatisticsContainer: React.FC = () => {
    ...
    } = useQuery(['totalCount'], () => JobOfferStatApis(t).getGlobalStatistics(JobStatEnum.TOTAL_COUNT));
    const {
        data: activeCount,
        isLoading: isLoadingActiveCount
    } = useQuery(['activeCount'], () => JobOfferStatApis(t).getGlobalStatistics(JobStatEnum.ACTIVE_COUNT));
    const {
        data: confirmedCount,
        isLoading: isLoadingConfirmedCount
    } = useQuery(['confirmedCount'], () => JobOfferStatApis(t).getGlobalStatistics(JobStatEnum.CONFIRMED_COUNT));
    const {
        data: expiredCount,
        isLoading: isLoadingExpiredCount
    } = useQuery(['confirmedCount'], () => JobOfferStatApis(t).getGlobalStatistics(JobStatEnum.EXPIRED_COUNT));

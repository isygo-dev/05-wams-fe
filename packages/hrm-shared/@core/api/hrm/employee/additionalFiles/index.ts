...
const EmployeeAdditionalFilesApis = (t: TFunction) => {
    ...
    const uploadAdditionalFiles = async (data: { parentId: number; files: Blob[] }) => {
        ...
        const response = await AppQuery(`${hrmApiUrls.apiUrl_HRM_Employee_MultiFilesUpload_EndPoint}?parentId=${data.parentId}`, {
            method: 'PUT',
            headers: {
    ...
    const downloadAdditionalFile = async (data: {
        ...
        const response = await AppQuery(`${hrmApiUrls.apiUrl_HRM_Employee_MultiFilesDownload_EndPoint}?parentId=${data.parentId}&fileId=${data.fileId}&version=${data.version}`, {
            method: 'GET',
            headers: {
                Accept: 'application/json',
                'Access-Control-Allow-Origin': '*'

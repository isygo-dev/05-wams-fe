...
const ContractApis = (t: TFunction) => {
  ...
  const updateStatusContract = async (data: RequestContractStatus) => {
    ...
    const response = await AppQuery(
      `${hrmApiUrls.apiUrl_HRM_Contract_UpdateStatus_EndPoint}?id=${data.id}&isLocked=${data.isLocked}`,
      {
        method: 'PUT',
        headers: {
          Accept: 'application/json',
  ...
  const downloadContractFile = async (data: { id: number; originalFileName: string }) => {
    ...
    const response = await AppQuery(`${hrmApiUrls.apiUrl_HRM_Contract_FileDownload_EndPoint}?id=${data.id}&version=1`, {
      method: 'GET',
      headers: {
        Accept: 'application/json',
        'Access-Control-Allow-Origin': '*'

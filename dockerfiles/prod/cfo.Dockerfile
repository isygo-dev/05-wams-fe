...
COPY ./apps/wams-cfo-fe ./apps/wams-cfo-fe
COPY ./apps/wams-cfo-fe/docker-prod.env ./apps/wams-cfo-fe/.env
...
RUN pnpm turbo run build:docker-prod --filter=apps/wams-cfo-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-prod", "--filter=apps/wams-cfo-fe..."]
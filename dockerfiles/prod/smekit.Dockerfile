...
COPY ./apps/wams-smekit-fe ./apps/wams-smekit-fe
COPY ./apps/wams-smekit-fe/docker-prod.env ./apps/wams-smekit-fe/.env
...
RUN pnpm turbo run build:docker-prod --filter=apps/wams-smekit-fe...

# Set working directory to the sysadmin frontend app
WORKDIR /app/apps/wams-smekit-fe

# Expose default port
EXPOSE 4009

# Start the frontend app using turbo filtered start
CMD ["pnpm", "turbo", "run", "start:docker-prod", "--filter=apps/wams-smekit-fe..."]
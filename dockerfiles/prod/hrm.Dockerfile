...
COPY ./apps/wams-hrm-fe ./apps/wams-hrm-fe
COPY ./apps/wams-hrm-fe/docker-prod.env ./apps/wams-hrm-fe/.env
...
RUN pnpm turbo run build:docker-prod --filter=apps/wams-hrm-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-prod", "--filter=apps/wams-hrm-fe..."]
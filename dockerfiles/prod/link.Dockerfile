...
COPY ./apps/wams-link-fe ./apps/wams-link-fe
COPY ./apps/wams-link-fe/docker-prod.env ./apps/wams-link-fe/.env
...
RUN pnpm turbo run build:docker-prod --filter=apps/wams-link-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-prod", "--filter=apps/wams-link-fe..."]
...
COPY ./apps/wams-quizz-fe/docker-prod.env ./apps/wams-quizz-fe/.env
...
RUN pnpm turbo run build:docker-prod --filter=apps/wams-quizz-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-prod", "--filter=apps/wams-quizz-fe..."]
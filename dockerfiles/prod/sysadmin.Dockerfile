...
COPY ./apps/wams-sysadmin-fe/docker-prod.env ./apps/wams-sysadmin-fe/.env
...
RUN pnpm turbo run build:docker-prod --filter=apps/wams-sysadmin-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-prod", "--filter=apps/wams-sysadmin-fe..."]
...
COPY ./apps/wams-rpm-fe ./apps/wams-rpm-fe
COPY ./apps/wams-rpm-fe/docker-prod.env ./apps/wams-rpm-fe/.env
...
RUN pnpm turbo run build:docker-prod --filter=apps/wams-rpm-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-prod", "--filter=apps/wams-rpm-fe..."]
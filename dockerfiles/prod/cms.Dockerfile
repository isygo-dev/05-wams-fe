...
COPY ./apps/wams-cms-fe/docker-prod.env ./apps/wams-cms-fe/.env
...
RUN pnpm turbo run build:docker-prod --filter=apps/wams-cms-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-prod", "--filter=apps/wams-cms-fe..."]
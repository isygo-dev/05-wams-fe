...
COPY ./apps/wams-cfo-fe/docker-dev.env ./apps/wams-cfo-fe/.env
...
RUN pnpm turbo run build:docker-dev --filter=apps/wams-cfo-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-dev", "--filter=apps/wams-cfo-fe..."]
...
COPY ./apps/wams-rpm-fe/docker-dev.env ./apps/wams-rpm-fe/.env
...
RUN pnpm turbo run build:docker-dev --filter=apps/wams-rpm-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-dev", "--filter=apps/wams-rpm-fe..."]
...
COPY ./apps/wams-gateway-fe/docker-dev.env ./apps/wams-gateway-fe/.env
...
RUN pnpm turbo run build:docker-dev --filter=apps/wams-gateway-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-dev", "--filter=apps/wams-gateway-fe..."]
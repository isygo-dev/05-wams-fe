...
COPY ./apps/wams-lms-fe/docker-dev.env ./apps/wams-lms-fe/.env
...
RUN pnpm turbo run build:docker-dev --filter=apps/wams-lms-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-dev", "--filter=apps/wams-lms-fe..."]

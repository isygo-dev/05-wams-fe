...
COPY ./apps/wams-link-fe/docker-dev.env ./apps/wams-link-fe/.env
...
RUN pnpm turbo run build:docker-dev --filter=apps/wams-link-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-dev", "--filter=apps/wams-link-fe..."]
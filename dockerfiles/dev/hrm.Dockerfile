...
COPY ./apps/wams-hrm-fe/docker-dev.env ./apps/wams-hrm-fe/.env
...
RUN pnpm turbo run build:docker-dev --filter=apps/wams-hrm-fe...
...
CMD ["pnpm", "turbo", "run", "start:docker-dev", "--filter=apps/wams-hrm-fe..."]